...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    if (action === "create") {
      ...
      const worker_code = "WRK" + Math.floor(100000 + Math.random() * 900000);
      const password = Math.random().toString(36).slice(-8) + "Wk!";
      ...
      // 2. Create worker profile in public.workers
    ...
    if (action === "delete") {
      const { id } = body;
      if (!id) {
        return new Response(JSON.stringify({ error: "Missing worker ID" }), { status: 400 });
      }

      // Delete from DB workers table (foreign key cascades or needs direct delete)
      const { error: dbError } = await supabaseServer
        .from("workers")
        .delete()
        .eq("id", id);
